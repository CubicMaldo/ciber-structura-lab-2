extends Control
## DFSVisualization - Visualización animada del algoritmo DFS
...
func _advance_dfs_step() -> void:
	...
	# Agregar vecinos a la pila (en orden inverso para mantener orden visual)
	var neighbors = []
	for edge in edges:
...
func _on_play_pressed() -> void:
	if is_animating:
		...
		if stack.is_empty() and visited.size() == nodes.size():
			_on_reset_pressed()
		is_animating = true
		if play_button:
